...
![image-20230502223203160](C:\Users\方锐\AppData\Roaming\Typora\typora-user-images\image-20230502223203160.png)
...
modify 的时候，一定要带上 字段名 数据类型 ，不然就会报错

如果只有 字段名 和 数据类型 的话，就是设置成默认值 Null (即可以插入Null)
alter table 表名称 modify 字段名 数据类型
```

![image-20230502230248235](C:\Users\方锐\AppData\Roaming\Typora\typora-user-images\image-20230502230248235.png)



`MODIFY` 语句中的 `column_name` 和 `new_data_type` 是必须的参数，因为它们用于指定要修改的列名称和新的数据类型。如果其中一个参数缺失，MySQL 将无法识别该语句，并返回一个错误。

例如，以下 SQL 语句省略了新数据类型：

```
ALTER TABLE employees MODIFY COLUMN age;
```

此语句会导致错误，因为它缺少必要的新数据类型参数。要成功执行修改操作，必须在语句中包含新的数据类型。

同样，如果省略列名称，则 MySQL 将无法确定要修改的列。因此，`column_name` 参数也是必需的。

总之，`MODIFY` 语句的语法必须完全符合 MySQL 的要求，才能正确地修改表格列定义。



```
当表中某列中有 Null 值，就不能更改这个 列 设置为其储存的值不能为 Null
```

![image-20230502225409871](C:\Users\方锐\AppData\Roaming\Typora\typora-user-images\image-20230502225409871.png)

![image-20230502225419455](C:\Users\方锐\AppData\Roaming\Typora\typora-user-images\image-20230502225419455.png)

你可以更改一个包含NULL值的列的默认值为NOT NULL，但在执行这个操作之前，你需要处理这个列中的NULL值。否则，MySQL将无法执行更改，因为这将违反NOT NULL约束。

要将一个包含NULL值的列更改为NOT NULL，你可以按照以下步骤操作：

1. 首先更新列中的NULL值。你可以将它们更改为默认值或其他适当的值。例如，如果这是一个整数列，你可以将NULL值更新为0：

   ```
   UPDATE your_table SET your_column = 0 WHERE your_column IS NULL;
   ```

2. 接下来，使用ALTER TABLE语句修改列的定义，将其更改为NOT NULL：

   ```
   ALTER TABLE your_table MODIFY your_column your_data_type NOT NULL;
   ```

   其中，your_table是你要修改的表的名称，your_column是你要修改的列的名称，your_data_type是你要使用的数据类型。

通过这两个步骤，你可以成功地将一个包含NULL值的列更改为NOT NULL。请注意，在执行此操作时确保对表进行备份，以防意外导致数据丢失。
